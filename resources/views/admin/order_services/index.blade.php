@extends('layouts.admin', ['title' => 'Order Services - bengcall'])

@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Order Services</h1>
            <p class="mt-1 text-sm text-slate-600">Kelola pesanan service.</p>
        </div>

        <div class="flex flex-wrap items-center gap-3">
            <form method="GET" action="{{ route('admin.order-services.index') }}" class="flex items-center gap-2">
                <input
                    type="text"
                    name="q"
                    value="{{ $search ?? '' }}"
                    ...
                    @forelse ($orders as $order)
                        ...
                        <tr class="text-sm text-slate-700">
                            ...
                            <td class="px-5 py-3 text-right align-top">
                                <a
                                    href="{{ route('admin.order-services.show', $order) }}"
