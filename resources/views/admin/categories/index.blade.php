@extends('layouts.admin', ['title' => 'Master Categories - bengcall'])

@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Master Categories</h1>
            <p class="mt-1 text-sm text-slate-600">CRUD untuk data category.</p>
        </div>
            ...
            <table class="min-w-full table-fixed divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-600">
                        ...
                        <th class="w-[280px] px-5 py-3">Slug</th>
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @forelse ($categories as $category)
                        <tr class="text-sm text-slate-700">
                            ...
                            <td class="px-5 py-3 align-top">
                                <p class="truncate text-slate-600" title="{{ $category->slug }}">{{ $category->slug }}</p>
                            </td>
                            <td class="px-5 py-3 align-top">{{ $category->products_count ?? 0 }}</td>
