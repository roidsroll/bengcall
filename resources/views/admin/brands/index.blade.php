@extends('layouts.admin', ['title' => 'Master Brands - bengcall'])

@section('content')
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Master Brands</h1>
            <p class="mt-1 text-sm text-slate-600">CRUD untuk data brand.</p>
        </div>
            ...
            <table class="min-w-full table-fixed divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr class="text-left text-xs font-semibold uppercase tracking-wider text-slate-600">
                        <th class="w-[300px] px-5 py-3">Nama</th>
                        <th class="w-[360px] px-5 py-3">Logo (URL)</th>
                        <th class="w-[140px] px-5 py-3">Products</th>
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @forelse ($brands as $brand)
                        <tr class="text-sm text-slate-700">
                            ...
                            <td class="px-5 py-3 align-top">
                                <p class="truncate text-slate-600" title="{{ $brand->logo ?? '' }}">{{ $brand->logo ?? '-' }}</p>
                            </td>
                            <td class="px-5 py-3 align-top">{{ $brand->products_count ?? 0 }}</td>
